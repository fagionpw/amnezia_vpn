#! /usr/bin/env bash
BRAND_NAME="VPN-BackComet-games"
# ==== Logging injection (non-destructive) ====
LOG_DIR="${PWD}/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/menu_$(date +"%Y%m%d_%H%M%S").log"
exec 19>>"$LOG_FILE"
export BASH_XTRACEFD=19
export PS4='+ [$(date "+%F %T")] ${FUNCNAME[0]:-main}:${LINENO}: '
set -o errtrace
trap 'ec=$?; echo "[ERROR] $(date "+%F %T") exit ${ec} at ${BASH_SOURCE[0]}:${LINENO} while running: ${BASH_COMMAND}" >&19' ERR
{
  echo "===== Amnezia setup started at $(date "+%F %T") ====="
  echo "PWD: $PWD"
...
log_compose_status() {
  local file="$1"
  {
    echo "----- docker compose ps for $file at $(date "+%F %T") -----"
    if command -v docker >/dev/null 2>&1; then
      docker compose -f "$file" ps 2>&1 || true
      echo "----- docker compose recent logs (last 100 lines) for $file -----"
      docker compose -f "$file" logs --no-color --tail=100 2>&1 || true
    else
...
script1() {
	...
	while true; do
		read -p "Enter the language for the Amnezia web interface (en, ua, ru, tr, no, pl, fr, de, ca, es, ko, vi, nl, is, pt, chs, cht, it, th, hi, ja, si): " lang
		if [ -z "$lang" ]; then
	...
	while true; do
		read -p "Enter your password and press Enter to access the Amnezia web interface: " password
		if [ -z "$password" ]; then
	...
	echo -e "${GREEN}Your password to access the Amnezia web interface:${NC}" "$password"
	...
	log_compose_status $compose_file
	sleep 2
	...
	log_compose_status $compose_file
                docker volume rm vpn-amnezia-wg-main_conf.d vpn-amnezia-wg-main_vhost.d vpn-amnezia-wg-main_html vpn-amnezia-wg-main_certs vpn-amnezia-wg-main_etc_wireguard &> /dev/null
                rm -r $compose_file &> /dev/null
                curl -L -O https://raw.githubusercontent.com/sergeybezlepkin/vpn-amnezia-wg/main/compose.yml &> /dev/null
                exit 1
    fi
	...
	log_compose_status $compose_beszel
	sleep 3
	echo
	...
	echo
	sleep 1
	echo -e "To access the server web panel Amnezia, use ${GREEN}https://$d_name1 and password: $password${NC}, go to the panel, log in with your password and create the first user"
	echo
	sleep 1
	...
	echo
	sleep 1
	echo -e "${GREEN}You can download the app for any device here: https://github.com/amnezia-vpn/amnezia-client/releases - Windows, Linux, Android, and look in the App store on your smartphone - AmneziaWG${NC}"
	echo
	sleep 1
	echo "Click on the 'Download' configuration icon and add it to the Amnezia VPN application on your computer"
	echo
	sleep 1
...
script2() {
	...
	while true; do
    		read -p "Enter your new password and press Enter to access the Amnezia web interface: " password
    		if [ -z "$password" ]; then
        		echo -e "${RED}You can't get into the panel without a password. Password cannot be empty. Try again${NC}"
    		elif [[ ! "$password" =~ ^[a-zA-Z0-9]+$ ]]; then
        		echo -e "${RED}Password must contain only english characters and numbers. Try again${NC}"
	...
	awk -v hash="$escaped_hash" '/^ *- PASSWORD_HASH=/ { sub(/- PASSWORD_HASH=.*/, "- PASSWORD_HASH=" hash) } 1' "$compose_file" > temp_compose.yml && mv temp_compose.yml "$compose_file"
	sleep 1
	docker rm -f amnezia &> /dev/null
	docker compose up -d &> /dev/null
	log_compose_status compose.yml
	sleep 3
	echo
	echo -e "${GREEN}Your new password to access the Amnezia web interface:${NC}" "$password"
	echo
...
menu() {
    echo -e "	Automatic installation and configuration of ${RED}Amnezia VPN server${NC} with web interface,"
    echo "      		 node monitoring, with encryption on a free domain name"
    echo
    echo -e "${GREEN}1. Install and configure VPN server Amnezia${NC}"
    echo
    ...
    echo
    echo -e "${GREEN}2. Reset web panel password Amnezia${NC}"
    echo
...
script7() {
        echo
        echo -e "${GREEN}Xray setup (VLESS/VMess over WebSocket via nginx-proxy)${NC}"
        echo

        default_domain="${XRAY_DOMAIN:-vpn.backkometagames.com}"
read -p "Enter domain for Xray (e.g. xray.example.com) [${default_domain}]: " xray_domain
xray_domain="${xray_domain:-$default_domain}"
        while [[ -z "$xray_domain" || ! "$xray_domain" =~ ^([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}$ ]]; do
                echo -e "${RED}Invalid domain. Try again (e.g. xray.example.com)${NC}"
                read -p "Enter domain for Xray: " xray_domain
        done
        echo "You entered domain: $xray_domain"

        read -p "Enter email for Let's Encrypt (leave empty to reuse previous): " le_email_input
        ...
        if [[ -z "$le_email" ]]; then
                read -p "Enter email for Let's Encrypt (required): " le_email
        fi
        echo "Email to use: $le_email"

        env_file="$PWD/.env"
        touch "$env_file"
        grep -q '^XRAY_DOMAIN=' "$env_file" && sed -i "s/^XRAY_DOMAIN=.*/XRAY_DOMAIN=$xray_domain/" "$env_file" || echo "XRAY_DOMAIN=$xray_domain" >> "$env_file"
        grep -q '^LE_EMAIL=' "$env_file" && sed -i "s/^LE_EMAIL=.*/LE_EMAIL=$le_email/" "$env_file" || echo "LE_EMAIL=$le_email" >> "$env_file"
        if ! grep -q '^XRAY_UUID=' "$env_file"; then
                XRAY_UUID=$(cat /proc/sys/kernel/random/uuid)
                echo "XRAY_UUID=$XRAY_UUID" >> "$env_file"
        else
                XRAY_UUID=$(grep '^XRAY_UUID=' "$env_file" | cut -d= -f2)
        fi
        echo "XRAY_UUID=$XRAY_UUID"

        mkdir -p "$PWD/xray"
        tmpl="$PWD/xray/config.json"
        if [ -f "$tmpl" ]; then
                sed -i "s/\${XRAY_UUID}/$XRAY_UUID/g" "$tmpl"
        fi

        docker compose -f $compose_file up -d xray &> /dev/null
        log_compose_status $compose_file

        echo
        echo -e "${GREEN}Xray is up (proxied via nginx at https://$xray_domain).${NC}"
        echo "Client URIs:"
        echo "  VLESS: vless://$XRAY_UUID@$xray_domain:443?encryption=none&security=tls&type=ws&path=%2Fvless#VPN-BackComet-games"
        echo "  VMess: vmess://$(echo -n "{\"v\":\"2\",\"ps\":\"amnezia-vmess\",\"add\":\"$xray_domain\",\"port\":\"443\",\"id\":\"$XRAY_UUID\",\"aid\":\"0\",\"net\":\"ws\",\"type\":\"none\",\"host\":\"$xray_domain\",\"path\":\"/vmess\",\"tls\":\"tls\"}" | base64 -w0)"
        echo
}
